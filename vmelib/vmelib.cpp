...
uintptr_t VMEBridge::getAddr(int handle, int size)
{
  ...
  mapped_data = (char *) mmap(NULL, size, PROT_WRITE | PROT_READ, MAP_SHARED, handle, 0);
...
int VMEBridge::rl(int image, unsigned int addr, unsigned int *data, int size)
{
  ...
  if (pread(vme_handle[image], data, size, (addr - vmeBaseAddr[image]) | 0x40000000) != size)
  {
    *Err << "Bus error reading at address 0x" << hex << addr << dec << ", image " << image << "!\n";
    return -1;
  }
...
int VMEBridge::wl(int image, unsigned int addr, unsigned int *data, int size)
{
  ...
  if (pwrite(vme_handle[image], data, size, (addr - vmeBaseAddr[image]) | 0x40000000) != size)
  {
    *Err << "Bus error writing at address 0x" << hex << addr << dec << ", image " << image << "!\n";
    return -1;
  }
...
int VMEBridge::rw(int image, unsigned int addr, unsigned short *data, int size)
{
  ...
  if (pread(vme_handle[image], data, size, (addr - vmeBaseAddr[image]) | 0x20000000) != size)
  {
    *Err << "Bus error reading at address 0x" << hex << addr << dec << ", image " << image << "!\n";
    return -1;
  }
...
int VMEBridge::ww(int image, unsigned int addr, unsigned short *data, int size)
{
  ...
  if (pwrite(vme_handle[image], data, size, (addr - vmeBaseAddr[image]) | 0x20000000) != size)
  {
    *Err << "Bus error writing at address 0x" << hex << addr << dec << ", image " << image << "!\n";
    return -1;
  }
...
int VMEBridge::rb(int image, unsigned int addr, unsigned char *data, int size)
{
  ...
  if (pread(vme_handle[image], data, size, (addr - vmeBaseAddr[image]) | 0x10000000) != size)
  {
    *Err << "Bus error reading at address 0x" << hex << addr << dec << ", image " << image << "!\n";
    return -1;
  }
...
int VMEBridge::wb(int image, unsigned int addr, unsigned char *data, int size)
{
  ...
  if (pwrite(vme_handle[image], data, size, (addr - vmeBaseAddr[image]) | 0x10000000) != size)
  {
    *Err << "Bus error writing at address 0x" << hex << addr << dec << ", image " << image << "!\n";
    return -1;
  }
...
int VMEBridge::there(unsigned int addr, unsigned int mode)
{
  ...
  case 1:
    return 1;
  case -1:
    *Err << "Address " << hex << addr << dec << " is not supported by any image!\n";
    return 0;
  case -2:
...
//  setup IRQ handling
//----------------------------------------------------------------------------
int VMEBridge::setupIrq(int image, unsigned int irqLevel, unsigned int statusID, unsigned int addrSt, unsigned int valSt, unsigned int addrCl, unsigned int valCl)
{
...
//  wait for IRQ
//----------------------------------------------------------------------------
int VMEBridge::waitIrq(unsigned int irqLevel, unsigned int statusID, uint32_t timeout)
{
...
int VMEBridge::generateVmeIrq(unsigned int irqLevel, unsigned int statusID)
{
  ...
  if (statusID & 1)
  {
    *Err << "Error! Due to a limitation of the universeII chip, software " << "generated VMEBus interrupts can only be performed with an " << "even statusID!\n(see docu for more information)\n";
    return -2;
  }
...
int VMEBridge::checkDmaParam(unsigned int count, unsigned int bufNr)
{
  if (count > dmaBufSize)
  {
    *Err << "DMA operation exceeds buffer size of " << dmaBufSize / 1024 << " kB!\n";
    return -1;
  }

  if (bufNr > dmaMaxBuf)
  {
    *Err << "DMAwrite: Buffer number too high! Only " << dmaMaxBuf + 1 << " buffer(s) available!\n";
    return -2;
  }
...
//----------------------------------------------------------------------------
int VMEBridge::DMAwrite(unsigned int dest, unsigned int count, int vas, int vdw, unsigned int bufNr)
{
  int offset;
  dma_param_t param;
...
//----------------------------------------------------------------------------
int VMEBridge::DMAread(unsigned int source, unsigned int count, int vas, int vdw, unsigned int bufNr)
{
  int offset;
  dma_param_t param;
...
int VMEBridge::DMAread(unsigned int source, unsigned int count, int vas, int vdw)
{
...
//----------------------------------------------------------------------------
unsigned int VMEBridge::addCmdPkt(int list, int write, unsigned int vmeAddr, int size, int vas, int vdw)
{
  unsigned int offset;
...
//----------------------------------------------------------------------------
int VMEBridge::vmemap(int image, unsigned int vme_base, unsigned int size, unsigned int ctl, int ms)
{
  int ret;
...
//  getImage
//----------------------------------------------------------------------------
int VMEBridge::getImage(unsigned int base, unsigned int size, int vas, int vdw, int ms)
{
  char vmeDev[80];
  int image;
  unsigned int ctl;
  ...
  {
    base &= 0xFFFF0000;
    *Std << "Warning: Base address must be 64k aligned! " << "Base address will be set to " << hex << base << dec << "!\n";
  }
